...
export function AlertsBanner({ alerts, riskUpdates, onSelectUser }: AlertsBannerProps) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-4 h-[170px] shrink-0 border border-[#222] bg-black/40 rounded-xl overflow-hidden flex flex-col backdrop-blur-md">
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#222] bg-[#0a0a0a]">
        <div className="flex items-center gap-2 text-[10px] font-mono font-bold tracking-widest text-[#888] uppercase">
          <AlertTriangle className="w-3.5 h-3.5 text-[#ff3366]" />
          <span>Active Critical Alerts</span>
        </div>
        <span className="text-[10px] font-bold text-[#555] font-mono">{alerts.length} PENDING</span>
      </div>
      
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 flex flex-col gap-2">
        {alerts.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-[#555] font-mono text-xs">
            <Shield className="w-5 h-5 mb-2 opacity-50 text-[#00ff88]" />
